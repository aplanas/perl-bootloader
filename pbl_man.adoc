...
The boot loader to act on is _LOADER_TYPE_ in _/etc/sysconfig/bootloader_. If
that is empty, the boot loader installed in the ESP is used instead; if none is
installed, nothing is done. _LOADER_TYPE="none"_ also does nothing.
...
*--no-nvram*::
Do not use the EFI variables: read the configuration files instead, as with
_UPDATE_NVRAM="no"_ in _/etc/sysconfig/bootloader_. Give it before the
...
*--get-default*::
Print the default boot entry.

*--get-timeout*::
Print the boot menu timeout, in seconds. A menu that is not shown is 0 and a
menu that waits forever is -1. Nothing is printed if no timeout is set.

*--needs-update*::
Compare the boot loader installed on the EFI System Partition with the one the
system has: print *update* if the system has a newer one, *newer* if the
installed one is newer, or *current* if they are the same. shim counts as part
of the answer, as it is installed together with the boot loader. Nothing is
printed if the two cannot be compared.

*--get-loader-path*::
Print the path of the boot loader binary on the EFI System Partition: the one
the firmware starts, or the one shim starts behind it. Nothing is printed if
there is none.
